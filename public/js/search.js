  ...
  $.ajax({
    ...
    success: function success(data, state) {
      ...
      $.ajax({
        ...
        success: function success(data) {
          if (data.results == 0) {// $('.empty h2').text('Nessun risulato trovato');
          } else {
            var lat = data.results[0].position.lat;
            var _long4 = data.results[0].position.lon;
            advanced(lat, _long4, beds, rooms, radius, wifi, smoking, parking, swimmingPool, breakfast, view);
          }
        },
...
function advanced(lat, _long5, beds, rooms, radius, wifi, smoking, parking, swimmingPool, breakfast, view) {
  $.ajax({
    ...
    success: function success(data, state) {
      if (data.flats.length > 0 || data.flatsPromo.length > 0) {
        if (data.flats <= 0) {// $('.empty h2').text('Nussun risulato trovato');
        } else {
          // ordiare i data.flats per distance
          data.flats.sort(function (a, b) {
            return a.distance - b.distance;
          ...
          for (var i = 0; i < data.flats.length; i++) {
            ...
            var template = Handlebars.compile(source);
            var flat = data.flats[i];
            var distance = Math.round(flat.distance * 100) / 100; // gestire gli extra services

            var arrayExtra = '';

            if (flat.name != undefined) {
              var flatService = flat.name.split(" "); // controllo se il wifi e' presente nel flat
        ...
        if (data.flatsPromo <= 0) {// $('.empty h2').text('Nussun risulato trovato');
        } else {
          // ordiare i data.flats per distance
          data.flatsPromo.sort(function (a, b) {
            return a.distance - b.distance;
          ...
          for (var i = 0; i < data.flatsPromo.length; i++) {
            ...
            var template = Handlebars.compile(source);
            var flatPromo = data.flatsPromo[i];
            var distancePromo = Math.round(flatPromo.distance * 100) / 100; // gestire gli extra services

            var arrayExtraPromo = '';

            if (flatPromo.name != undefined) {
              var flatService = flatPromo.name.split(" "); // controllo se il wifi e' presente nel flat
    ...
    error: function error(request, state, _error5) {
      console.log(_error5);
    }
  });
}
...
module.exports = __webpack_require__(/*! /opt/lampp/htdocs/boolbnb-progetto/resources/js/search.js */"./resources/js/search.js");
